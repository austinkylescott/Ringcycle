...
var _species: Dictionary = {}   # id -> WrestlerSpeciesResource
var _templates: Dictionary = {} # name -> WrestlerResource


func _ready() -> void:
	_load_species_folder("res://scripts/data/species")
	print("[SpeciesRegistry] loaded %d species and %d templates: %s" % [
		_species.size(), _templates.size(), _species.keys()
	])
